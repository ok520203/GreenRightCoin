...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 17254
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 17354
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 117254
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 117354
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 17254
	%{_sbindir}/semanage port -d -p tcp 17354
	%{_sbindir}/semanage port -d -p tcp 117254
	%{_sbindir}/semanage port -d -p tcp 117354
	for selinuxvariant in %{selinux_variants}; do
