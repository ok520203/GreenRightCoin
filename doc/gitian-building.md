...
*Setup instructions for a Gitian build of GreenRightCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the GreenRightCoin
...
- [Building GreenRightCoin Core](#building-greenrightcoin-core)
...
Clone the git repositories for greenrightcoin and Gitian.
...
in the greenrightcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building GreenRightCoin Core
----------------

To build GreenRightCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the greenrightcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/greenrightcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/greenrightcoin/greenrightcoin
...
For example:
```bash
URL=https://github.com/thrasher-/greenrightcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit greenrightcoin=${COMMIT} --url greenrightcoin=${URL} ../greenrightcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit greenrightcoin=${COMMIT} --url greenrightcoin=${URL} ../greenrightcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit greenrightcoin=${COMMIT} --url greenrightcoin=${URL} ../greenrightcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the greenrightcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../greenrightcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/greenrightcoin
SIGPATH=/some/root/path/greenrightcoin-detached-sigs

./bin/gbuild --url greenrightcoin=${BTCPATH},signature=${SIGPATH} ../greenrightcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/greenrightcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/greenrightcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/greenrightcoin-osx-build.assert
```
...
[greenrightcoin/gitian.sigs.ltc](https://github.com/greenrightcoin/gitian.sigs.ltc/) repository, or if that's not possible create a pull
